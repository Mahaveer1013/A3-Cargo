...
const AdminLayout = ({ children }) => {
  ...
  if (!isAuthenticated) {
    ...
    return (
      ...
      <>
        <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-blue-400 via-indigo-500 to-purple-600 relative">
          {/* <h2 style={{ position: 'absolute', top: '20px', left: '20px', color: 'black' ,fontSize:"3rem"}}>
            We are more than 
          </h2>
          <h2 style={{ position: 'absolute', top: '70px', left: '20px', color: 'black' ,fontSize:"3rem"}}>
            just a shipping partner
          </h2> */}
            ...
            <form onSubmit={handleLogin}>
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  className="mt-2 block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-300 ease-in-out"
                />
              </div>
              {error && <div className="text-red-500 text-sm mb-4">{error}</div>}
              <button
                type="submit"
                className="w-full py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none transition duration-200 ease-in-out transform hover:scale-105"
              >
                Submit
              </button>
            </form>
